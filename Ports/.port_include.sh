...
addtodb() {
  ...
  if ! grep -E "^(auto|manual) $port $version" "$prefix"/packages.db >/dev/null; then
    ...
    echo >&2 "Warning: $port $version already installed. Not adding to database of installed ports!"
  fi
}
installdepends() {
...
uninstall() {
  if grep "^manual $port " "$prefix"/packages.db >/dev/null; then
    if [ -f plist ]; then
      ...
      echo >&2 "Error: This port does not have a plist yet. Cannot uninstall."
    fi
  else
    echo >&2 "Error: $port is not installed. Cannot uninstall."
  fi
}
do_fetch() {
  installdepends
...
if [ -z "${1:-}" ]; then
  ...
  case "$1" in
    ...
    --auto)
      ...
      ;;
    *)
      echo >&2 "I don't understand $1! Supported arguments: fetch, configure, build, install, clean, clean_dist, clean_all, uninstall."
      exit 1
      ;;
  esac
fi
